...
###
puts "\n\n#####################################################################"
puts "HEY THERE, THIS PROGRAM DOESN'T WORK YET, OPTIONS B+C+F+G ARE BUGGY, IT PAINS ME BUT I JUST DIDN'T HAVE ENOUGH TIME TO FIX IT."
...
while stay_in_loop
  ...
  when "E"
    ...
    puts hotel.get_cost(res_id.to_i)
  ...
  when "G"
    ...
    room_id = hotel.prompt_for_input(statement: "What is the room id?")
    customer = hotel.prompt_for_input(statement: "What's the customer's name?")
    new_res = hotel.make_reservation_from_block(room_id:room_id.to_i, customer:customer)
    puts "CONFIRM: #{new_res}"
